[README (8).md](https://github.com/user-attachments/files/31372700/README.8.md)
# EvidenceGuard

> **Research prototype — Phase 2 evaluated**
>
> EvidenceGuard is an experimental system for trustworthy and explainable
> scientific evidence synthesis using Retrieval-Augmented Generation (RAG).
>
> The project evaluates retrieval, citation attribution, abstention,
> claim-level evidence verification, and confidence as separate components
> rather than treating answer reliability as a single score.

## Overview

EvidenceGuard investigates whether evidence-grounded generation and
post-generation verification can improve the reliability of LLM-generated
scientific answers.

> **Research question:** Can retrieval quality, evidence attribution,
> claim-level verification, abstention, and uncertainty signals improve the
> reliability of LLM-generated answers in scientific evidence synthesis?
...
Cross-encoder reranking produced the strongest ranking quality on the pilot
retrieval benchmark.
...
## Phase 2 — Generation and Verification Results

Evaluation was performed on a manually audited 11-question benchmark:
...
All citations generated for answered questions referred to valid chunk
identifiers available to the system.

**Citation grounding = 1.00**

Every answered benchmark question cited at least one passage included in the
manually audited gold evidence.

**Claim verification support = 0.829**

Approximately 83% of generated factual claims passed claim-level evidence
verification. Unsupported claims can be removed by the post-generation
verification layer.

**Abstention**

EvidenceGuard answered 8 of 9 answerable questions and abstained on one
answerable multi-document question.

Both intentionally unanswerable questions were correctly rejected. No
intentionally unanswerable question received a generated answer.
...
During evaluation, manual evidence auditing identified:

- valid alternative evidence missing from some original gold labels;
...
This prevents the system from being penalized for citing correct evidence that
was absent from the original annotations.

## Verification Design

Claim verification uses a Natural Language Inference cross-encoder.

The verification pipeline also includes:
...
Regression testing identified cases where abbreviation mismatch or insufficient
context caused false negatives. These cases were converted into automated
tests.

An experimental NLI-based reference-alignment metric was also investigated
during development but was excluded from headline results because validation
showed unacceptable false-negative behavior.
...
The final evaluation uses the manually audited benchmark and writes detailed
...
The results therefore characterize this prototype and pilot benchmark rather
than establish general performance across scientific evidence synthesis.
